...
if isMac():
    ...
    def getSysPath():
        command = "/bin/bash -l -c env $PATH | grep -e '^PATH='"

        env_string = Popen(command, stdout=PIPE, shell=True, env=originalEnv).stdout.read().decode('utf-8')
        print(env_string)
        if env_string:
            sysPathString = env_string.split('=')[-1]
    ...
    def fixPath():
        ...
        print('Adding path from system: %s' % currSysPath)
